...
```
modalities/
├── asl/                   # ASL-specific processing
│   ├── processor.py       # Main ASL processor
...
├── ge/                    # GE vendor sequences
│   ├── asl/               # GE ASL sequences
│   └── dsc/               # GE DSC sequences
├── siemens/               # Siemens vendor sequences
└── philips/               # Philips vendor sequences
```
...
```
User Upload → Frontend Validation → Backend API → File Storage → Core Processing
```

### 2. Report Generation Process
```
Core Processing → Validation → Report Generation → API Response →  Display
```

### 3. PDF Export Process
```
Report Data → HTML Template → WeasyPrint → PDF File → Download
```
